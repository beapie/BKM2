                                    ...
                                    <ul class="compare-plan-opction p-0">
                                        @foreach ($modules as $module)
                                            @php
                                                ...
                                                $is_theme = in_array($module, get_bussiness_theme_list());
                                            @endphp
                                            @if (!isset($json['display']) || $json['display'] == true)
                                                <li>
                                                    @if ($is_theme)
                                                        <a href="https://workdo.io/product-category/bookinggo-saas-addon/bookinggo-theme/" target="_new">{{ Module_Alias_Name($module->getName()) }}</a>
                                                    @else
                                                        <a target="_new" href="{{ route('software.details', Module_Alias_Name($module->getName())) }}">{{ Module_Alias_Name($module->getName()) }}</a>
                                                    @endif
                                                </li>
                                            @endif
                                        @endforeach
