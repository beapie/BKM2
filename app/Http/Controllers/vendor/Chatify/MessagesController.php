...
class MessagesController extends Controller
{
    ...
    public function getContacts(Request $request)
    {
        ...
        if($users->count() > 0)
        {
            ...
            foreach($users as $user)
            {
                ...
                {
                    // Get user data
                    $userCollection = User::where('id', $user->id)->where('business_id',getActiveBusiness())->first();
        ...
        {
            $members = User::where('created_by', '=', $objUser->id)->where('business_id',getActiveBusiness())->get();
        }
        else
        {
            $members = User::where('business_id',getActiveBusiness())->where('type', '!=', 'client')->where('type', '!=', 'vendor')->where('created_by', '=', $objUser->created_by)->where('id', '!=', $objUser->id)->orWhere('id', '=', $objUser->created_by)->get();
        }
        $getRecords = null;
    ...
    public function getFavorites(Request $request)
    {
        ...
        {
            // get user data
            $user          = User::where('id', $favorite->favorite_id)->where('business_id',getActiveBusiness())->first();
            if(!empty($user))
            {
                $favoritesList .= view(
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('created_by', $id)->where('type', '!=', 'client')->where('business_id',getActiveBusiness())->where('name', 'LIKE', "%{$input}%")->get();
