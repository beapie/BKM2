...
class BlogController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(Auth::user()->isAbleTo('blog create'))
        {
            $validator = \Validator::make(
                ...
                $blog->business_id             = getActiveBusiness();
    ...
    public function blogManage(Request $request,$id)
    {
        if(Auth::user()->isAbleTo('blog manage'))
        {
            $blogs = Blog::where('business_id',getActiveBusiness())->where('created_by',creatorId())->where('theme',$id)->get();
            return view('blog.index',compact('id','blogs'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function blogCreate(Request $request,$id)
    {
